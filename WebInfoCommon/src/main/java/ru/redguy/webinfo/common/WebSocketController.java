...
public class WebSocketController {
    ...
    private class Client extends WebSocketClient {
        ...
        public void onMessage(String message) {
            if ("request".equals(GSON.gson.fromJson(message, WebSocketMessage.class).getType())) {
                ...
                if (pages.containsKey(req.getMethod(), url)) {
                    ...
                    for (QueryArgument arg : page.getAnnotation().args()) {
                        if (!req.getParameters().containsKey(arg.name()) && arg.required()) {
                            genResponse(req.getId(), NanoHTTPD.Response.Status.BAD_REQUEST, Response.TheVariableIsNotPassed(arg.name()));
                            break;
                        }
                        switch (arg.type()) {
                            case UUID: {
                                ...
                                if (req.getParameters().get(arg.name()) != null)
                                    for (String s : req.getParameters().get(arg.name())) {
                                        try {
                                            ...
                                            genResponse(req.getId(), NanoHTTPD.Response.Status.BAD_REQUEST, Response.VariableIncorrect(arg.name()));
                                            break;
                                        }
                                    }
                    ...
                    try {
                        ...
                        genResponse(req.getId(), NanoHTTPD.Response.Status.BAD_REQUEST, Response.UnsupportedOperation());
                        return;
                    } catch (Exception e) {
                        e.printStackTrace();
                        genResponse(req.getId(), NanoHTTPD.Response.Status.INTERNAL_ERROR, Response.InternalError());
                        return;
                    }
                } else {
                    genResponse(req.getId(), NanoHTTPD.Response.Status.NOT_FOUND, Response.MethodNotFound());
                }
            }
        }
        ...
        public void genResponse(int id, NanoHTTPD.Response.@NotNull Status status, ru.redguy.webinfo.common.utils.Response response) {
